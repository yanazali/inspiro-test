...
export default class Machine extends React.Component {
  ...
  pay() {
    service
      .updateStock(this.state.productType)
      .then(res => {
  ...
  reset() {
    this.setState({ paid: false, productType: null });
  }
  render() {
    ...
    return (
      <div className="panel panel-danger text-center">
        <div className="panel-heading">
          <h4 className="panel-title">Vending Machine</h4>
        </div>
        ...
        <div className="panel-footer">
          {!this.state.productType && !this.state.busy && (
            <p className="status">Select Product</p>
          )}
          ...
          {this.state.paid && !this.state.busy && (
            <p className="status">ENJOY!</p>
          )}
        </div>
      </div>
    );
